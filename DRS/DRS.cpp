...
void DRSBoard::ConstructBoard()
{
   ...
   fStatusSource = ((bits & BIT_TR_SOURCE1) > 0) | (((bits & BIT_TR_SOURCE2) > 0) << 1);
...
void DRSBoard::PrintRegisters(FILE* f)
{
  unsigned char buffer[2];
  unsigned short d;


        fprintf(f,"STAT REG % 34sCTRL REG\n", "");
        for (int i=0x0; i<0x40 ; i+=0x2)
        {
        Read(T_STATUS, buffer, i, 2);

        d = (static_cast < unsigned int >(buffer[1]) << 8) +buffer[0];

        fprintf(f,"0x%04x  ",i);
        dec2bin(buffer[0],8); fprintf(f,"  ");
        dec2bin(buffer[1],8); fprintf(f,"  % 6d  %04x   ",d,d);

        Read(T_CTRL, buffer, i, 2);

        d = (static_cast < unsigned int >(buffer[1]) << 8) +buffer[0];

        fprintf(f,"0x%04x  ",i);
        dec2bin(buffer[0],8); fprintf(f,"  ");
        dec2bin(buffer[1],8); fprintf(f,"  % 6d  %04x\n",d,d);
        }
}
...
int DRSBoard::SetStatusSource(int source)
{
   short int reg;

   fStatusSource = source;
   if (fBoardType == 8 || fBoardType == 9) {
      // Set trigger configuration
/*
FW version: 533E
...
*/
      reg = (unsigned short) source;
      Write(T_CTRL, REG_STAT_CONFIG, &reg, 2);
   }
...
int DRSBoard::GetFiredTrigger()
{
   unsigned d, bit;
   Read(T_STATUS, &d, REG_FIRED_TRIG, 2);
   printf("%u\n", d);
   if ((d & (1 << 0))>0) printf("A");
   if ((d & (1 << 1))>0) printf("B");
   if ((d & (1 << 2))>0) printf("C");
...
void DRSBoard::SetLatchDelay(int delay)
{
   /* Set latch time to increase the readout time.
      0 = no latch
      (1..255)*16.667ns (ABCD,EXT) */
   unsigned int ticks, reg;

   ticks = (unsigned int)(delay);
...
//   Read(T_CTRL, &reg, REG_LATCH_DELAY, 2);
   //printf("TEST %us\n", reg);
   reg = ticks;
   //reg = 0x0FFF;
   //reg = 0xFFFF;
   Write(T_CTRL, REG_LATCH_DELAY, &reg, 2);

   fLatchDelay = ticks;
   fLatchDelayNs = 16.667 * ticks ;
}
/*------------------------------------------------------------------*/

void DRSBoard::SetLatchDelayNs(int delay)
{
   /* Set latch time to increase the readout time.
      0 = no latch
      (1..255)*16.667ns (ABCD,EXT) */
   ...
   ticks = (unsigned int)(delay / 16.667);
...
//int DRSBoard::GetLatchDelay()
//{
//   unsigned int delay;
//   Read(T_CTRL, &delay, REG_LATCH_DELAY, 2);
//   return static_cast < unsigned int > (delay);
//}



void DRSBoard::SetFixedRateDelay(int delay)
{
   /* Set Fixed Trigger Rate in 2^19*16.666ns steps , or 0 for OFF*/
   unsigned int ticks, reg;
...
//   Read(T_CTRL, &reg, REG_LATCH_DELAY, 2);
   ...
   //reg = 0x0FFF;
   //reg = 0xFFFF;
   Write(T_CTRL, REG_FIXED_RATE, &reg, 2);

   fFixedRateDelay = ticks;
   fFixedRateDelayNs = 16.667 * 524288 * ticks ;
}
/*------------------------------------------------------------------*/


unsigned long long DRSBoard::GetFirstTriggerTime()
{
   unsigned short buffer[4];
   Read(T_STATUS, &buffer[1], REG_TIME_CO0, 2);
   Read(T_STATUS, &buffer[0], REG_TIME_CO1, 2);
   Read(T_STATUS, &buffer[3], REG_TIME_CO2, 2);
   Read(T_STATUS, &buffer[2], REG_TIME_CO3, 2);
//   dec2bin(buffer[0],8); printf(" ");
//   dec2bin(buffer[1],8); printf(" ");
//   dec2bin(buffer[2],8);  printf(" ");
//   dec2bin(buffer[3],8);  printf(" ");
...
//   if (fBoardType < 8 || fFirmwareVersion < 17663 || fTransport != TR_USB2)
//      return 1;
   ...
   return static_cast < unsigned int >(d * 1.); // measurement clock is 10 Hz
}
...
int DRSBoard::CalibrateTiming(DRSCallback *pcb)
{
   ...
   SetReadoutMode(1);
   EnableTrigger(0, 0);
 // miesto 7 za 6 a 8 za 7

   if (fBoardType == 5 ) {
      EnableTcal(1, 0, 0);
      SelectClockSource(1); // 2nd quartz
   } else if (fBoardType == 8 || fBoardType == 7) {
      nIterSlope  = 0;
      nIterPeriod = 1500;
      EnableTcal(1, 0, 0);
      SelectClockSource(1); // 2nd quartz
